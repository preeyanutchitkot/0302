...
import {
  View, Text, TextInput, ScrollView, TouchableOpacity, StyleSheet, ActivityIndicator, Alert, KeyboardAvoidingView, Platform, Image
} from 'react-native';
...
const AppealChatScreen = () => {
  ...
  const [messages, setMessages] = useState([
    { id: Date.now().toString(), type: 'bot', text: '👋 คุณต้องการแจ้งปัญหาอะไร? (เลือกจาก ขยะ, ถนนพัง, น้ำท่วม, ไฟฟ้า, น้ำประปา, ฝาถัง, ไฟไหม้, การจราจร, ปัญหาทั่วไป)' }
  ]);
  ...
  const resetForm = () => {
    ...
    setImage(null);
    setLocation(null);
    setMessages([{ id: Date.now().toString(), type: 'bot', text: '👋 คุณต้องการแจ้งปัญหาอะไร? (เลือกจาก ขยะ, ถนนพัง, น้ำท่วม, ไฟฟ้า, น้ำประปา, ฝาถัง, ไฟไหม้, การจราจร, ปัญหาทั่วไป)' }]);
  };
  ...
  const handleCategorySelection = (selectedCategory) => {
    ...
    setMessages(prev => [
      ...prev,
      { id: Date.now().toString(), type: 'user', text: selectedCategory },
      { id: Date.now().toString(), type: 'bot', text: `กรุณาระบุรายละเอียดเพิ่มเติมเกี่ยวกับ ${selectedCategory}` }
    ]);
    setStep(1);
  };
  ...
  const handleSend = async () => {
    ...
    if (step === 1) {
      setDetail(text);
      setMessages(prev => [...prev, { id: Date.now().toString(), type: 'bot', text: '📷 ต้องการแนบรูปภาพไหม? (พิมพ์ "ใช่" หรือ "ไม่")' }]);
      setStep(2);
    } else if (step === 2) {
      if (text.toLowerCase().includes('ใช')) {
        pickImage();
      } else {
        setMessages(prev => [...prev, { id: Date.now().toString(), type: 'bot', text: '📍 ต้องการแชร์ตำแหน่งของคุณหรือไม่? (พิมพ์ "แชร์" หรือ "ไม่")' }]);
        setStep(3);
      }
    } else if (step === 3) {
      if (text.toLowerCase().includes('แชร์')) {
        ...
        setMessages(prev => [...prev, { id: Date.now().toString(), type: 'bot', text: '❌ คุณไม่ได้แชร์ตำแหน่ง กรุณาระบุจุดเกิดเหตุในข้อความค่ะ' }]);
        analyzeAndSubmit();
      }
    }
  };
...
const pickImage = async () => {
  ...
  if (!result.canceled) {
    ...
    setMessages(prev => [...prev, { id: Date.now().toString(), type: 'user', image: uri }]);

    try {
      const imageUrl = await uploadImageToCloudinary(uri);
      setImage(imageUrl); // เก็บไว้ด้วยถ้าอยากใช้ต่อ
    } catch (e) {
  ...
  setMessages(prev => [...prev, {
    ...
    type: 'bot',
    text: '📍 ต้องการแชร์ตำแหน่งของคุณหรือไม่? (พิมพ์ "แชร์" หรือ "ไม่")'
  }]);
  setStep(3);
};
  ...
  if (status !== 'granted') {
    setMessages(prev => [...prev, { id: Date.now().toString(), type: 'bot', text: '❌ ไม่ได้รับสิทธิ์เข้าถึงตำแหน่ง กรุณาระบุจุดเกิดเหตุในข้อความค่ะ' }]);
    analyzeAndSubmit();
    return;
  }
  ...
  try {
    ...
    setLocation(coords);
    setMessages(prev => [...prev, { id: Date.now().toString(), type: 'user', text: `📍 ตำแหน่งของคุณ: ${fullAddress}` }]);
    analyzeAndSubmit(null, coords); // 👈 ส่งตรงไป
  } catch (err) {
    setMessages(prev => [...prev, { id: Date.now().toString(), type: 'bot', text: '❌ เกิดข้อผิดพลาดขณะดึงตำแหน่ง' }]);
    analyzeAndSubmit();
  }
};
